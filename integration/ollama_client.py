...
class OllamaClient:
    ...
    def _check_model_availability(self):
        """Warn if model specific model is missing"""
        try:
            resp = requests.get(f"{self.base_url}/api/tags", timeout=2)
            if resp.status_code == 200:
                models = [m['name'] for m in resp.json().get('models', [])]
                # Check for exact match or simple name match (e.g. 'llama3.1:latest')
                if not any(self.model in m for m in models):
                    logger.warning(f"⚠️ Model '{self.model}' not found in Ollama. Run 'setup_ollama.bat' to pull it.")
                    logger.info("Falling back to first available model if possible...")
                    if models:
                        self.model = models[0]
