...
# ============================================================
# BRAIN MONITOR API (Observability Layer)
# Real-time brain state for the live dashboard
# ============================================================

# Lazy-init heartbeat (singleton)
_heartbeat = None
...
def get_heartbeat():
    ...
    if _heartbeat is None:
        ...
        # Register components
        _heartbeat.register("storage", manager.storage)
        from utils.metrics import MetricsTracker
        _heartbeat.register("metrics", MetricsTracker())
    return _heartbeat


@app.get("/api/brain/snapshot")
def get_brain_snapshot():
    """Current brain state — single point in time"""
    hb = get_heartbeat()
    return {
        "snapshot": hb.get_current(),
        "health": hb.get_health(),
    }


@app.get("/api/brain/history")
def get_brain_history(last_n: int = 60):
    """Time-series history for all metrics (for charting)"""
    hb = get_heartbeat()
    return {
        "history": hb.get_history(last_n),
        "total_ticks": hb.tick_count,
    }


@app.get("/api/brain/metric/{metric_name}")
def get_brain_metric(metric_name: str, last_n: int = 60):
    """Single metric time-series (for individual charts)"""
    hb = get_heartbeat()
    allowed = [
        "awake_hz", "awake_queue", "avg_quality", "avg_consistency",
        "total_engrams", "proofs_total", "refinements_total",
        "errors_total", "low_consistency_count", "axiom_derived_count",
        "gate_confidence", "cpu_percent", "memory_mb",
    ]
    if metric_name not in allowed:
        return {"error": f"Unknown metric. Available: {allowed}"}
    
    return {
        "metric": metric_name,
        "series": hb.get_time_series(metric_name, last_n),
    }


@app.get("/api/brain/health")
def get_brain_health():
    """Overall brain health + circuit breaker status"""
    hb = get_heartbeat()
    return hb.get_health()


@app.post("/api/brain/start")
def start_heartbeat():
    """Start the heartbeat (begin collecting metrics)"""
    hb = get_heartbeat()
    hb.start()
    return {"message": "Heartbeat started", "status": "running"}
...
def stop_heartbeat():
    ...
    hb = get_heartbeat()
    hb.stop()
    return {"message": "Heartbeat stopped"}


@app.websocket("/ws/brain")
async def brain_websocket(websocket: WebSocket):
    """Live WebSocket stream of brain snapshots (1 Hz)"""
    await ws_manager.connect(websocket)
    hb = get_heartbeat()
